...
export default function MissionsScreen() {
  ...
  const handleCreate = () => {
    ...
    const mission = addMission(newTitle.trim(), newDesc.trim())
    // Auto-generate first micro-step so user has something immediately actionable
    try {
      const result = compileMission({
        ...
        energy: 'medium',
        availableMinutes: 5,
        contextText: newTitle.trim(),
        threadId: null,
        previousFailures: [],
        previousSuccesses: [],
        protocolId: getProtocolForState('avoiding'),
        privacyPolicy: 'local_only',
      })
      useAppStore.getState().addMicroMission(
        mission.id,
      ...
      // If compilation fails, mission is still created — user can AI-breakdown later
    }
    setNewTitle('')
    setNewDesc('')
    setShowCreate(false)
  ...
  return (
    <Screen gradient={['rgba(108,58,237,0.03)', 'transparent']}>
      ...
      <View style={{ height: activeMissions.length * 120 }}>
        <FlashList
          data={activeMissions}
          ...
          renderItem={({ item: mission }) => {
            ...
            return (
              <Card variant={isExpanded ? 'glow' : 'default'} style={styles.missionCard}>
                <TouchableOpacity onPress={() => setExpandedMission(isExpanded ? null : mission.id)} activeOpacity={0.8}>
                  <View style={styles.missionHeader}>
                    ...
                    <View style={styles.missionInfo}>
                      ...
                      {(() => {
                        const daysSinceCreated = Math.floor((Date.now() - new Date(mission.created_at).getTime()) / 86400000)
                        const isStale = daysSinceCreated >= 7
                        return isStale ? <Text style={{ ...typography.caption, color: colors.accent.orange }}>{daysSinceCreated}d ago</Text> : null
                      })()}
